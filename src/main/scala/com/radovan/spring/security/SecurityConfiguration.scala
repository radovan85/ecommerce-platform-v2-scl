...
class SecurityConfiguration {
  ...
  protected def configure(http: HttpSecurity): SecurityFilterChain = {
    ...
    http.authorizeHttpRequests
      ...
      .requestMatchers("/loginErrorPage", "/products/**", "/aboutUs", "/suspensionPage").permitAll
      .requestMatchers("/", "/home", "/registerComplete", "/registerFail", "/loginPassConfirm", "/suspensionChecker").permitAll
      .requestMatchers("/get*").hasAnyAuthority("ADMIN", "ROLE_USER")
      .requestMatchers("/admin/**").hasAuthority("ADMIN")
      .requestMatchers("/cart/**", "/order/**").hasAuthority("ROLE_USER")
      .requestMatchers("/userRegistration").anonymous
  ...
  @Bean def resourcesCustomizer: WebSecurityCustomizer = (web: WebSecurity) =>
